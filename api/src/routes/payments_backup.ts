...
export async function paymentsRoutes(app: FastifyInstance) {
  app.post('/payments', { preHandler: [idempotencyGuard] }, async (req, reply) => {
    ...
    const payment = await prisma.payment.create({
      data: {
        merchantId: body.merchantId ?? 'demo_merchant',
        amount: body.amount,
    ...
    if (body.method === 'bank') {
      ...
      if ((body.provider_pref ?? 'nmi') === 'nmi') {
        attemptResp = await nmiCharge({ token: body.token ?? 'tok_stub', amount: body.amount, currency: body.currency, capture: body.capture });
      } else {
        attemptResp = await authNetCharge({ token: body.token ?? 'tok_stub', amount: body.amount, currency: body.currency, capture: body.capture });
      }
    ...
    const saved = await prisma.payment.update({ where: { id: payment.id }, data: { status } });

    // Persist idempotent response if key provided
  const key = req.headers['idempotency-key'];
    if (key && typeof key === 'string') {
    await prisma.idempotency.upsert({
    where: { key },
    create: { key, responseJson: { payment_id: saved.id, status: saved.status, provider: saved.provider } },
    update: { responseJson: { payment_id: saved.id, status: saved.status, provider: saved.provider } }
  });
}

  app.get('/payments/:id', async (req, reply) => {
    const id = (req.params as any).id as string;
    const payment = await prisma.payment.findUnique({ where: { id }, include: { attempts: true, refunds: true } });
  ...
  app.post('/payments/:id/refunds', async (req, reply) => {
    ...
    const refund = await prisma.refund.create({ data: { paymentId: id, amount, status: 'pending' } });
