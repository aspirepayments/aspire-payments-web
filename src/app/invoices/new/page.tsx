...
export default function NewInvoicePage(){
  ...
  const { data: itemData } = useSWR<{items:Item[]}>('/items', apiGet);
  ...
  // lines
  type Line = { id:string; itemId?:string; description:string; qty:number; unitPrice:number; taxable:boolean };
  const [lines, setLines] = useState<Line[]>([]);
  const addLine    = ()=> setLines(l => [...l, { id: crypto.randomUUID(), description:'', qty:1, unitPrice:0, taxable:false }]);
  ...
  const previewFee = useMemo(()=>{
    if(!selectedPlan) return { amount:0, description:'', mode:'none' as 'none'|'convenience'|'service' };
    const mode = String(selectedPlan.mode || 'none').toLowerCase() as 'none'|'convenience'|'service';
    if(mode === 'convenience'){
      const cents = Math.max(0, Number(selectedPlan.convenienceFeeCents||0)|0);
      return cents>0 ? { amount:cents, description:selectedPlan.name || 'Convenience Fee', mode } : { amount:0, description:'', mode };
    }
    if(mode === 'service'){
      const bps = Math.max(0, Number(selectedPlan.serviceFeeBps||0)|0);
      const amt = Math.round(baseSubtotal * (bps / 10000));
      return amt>0 ? { amount:amt, description:`${selectedPlan.name || 'Service Fee'} (${(bps/100).toFixed(2)}%)`, mode } : { amount:0, description:'', mode };
    }
    return { amount:0, description:'', mode:'none' };
  ...
  const matchItemByName = (name: string) => items.find(i => i.name.toLowerCase() === name.toLowerCase());
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <Card className="p-4 space-y-4">
        ...
        {/* Fee + Tax */}
        {(plans.length>0) && (
          <label className="block text-sm"><span>Fee plan</span>
                  ...
                  <tr key={l.id}>
                    <td>
                      <div className="flex gap-2">
                        <input
                          className="input"
                          placeholder="Description or select from catalog"
                          value={l.description}
                          onChange={e=>{
                            const name=e.target.value;
                            updateLine(l.id,{description:name});
                            const found = matchItemByName(name.trim());
                            if(found) updateLine(l.id,{itemId:found.id, unitPrice:found.unitPrice});
                          }}
                        />
                        <Btn kind="ghost" className="text-xs px-2"
                          onClick={async()=>{
                            const name=(l.description||'').trim();
                            if(!name){ alert('Enter description first'); return; }
                            const r = await apiPost<{item:Item}>('/items',{ name, unitPrice:l.unitPrice, merchantId:'demo_merchant' });
                            updateLine(l.id,{ itemId:r.item.id, description:r.item.name, unitPrice:r.item.unitPrice });
                          }}>
                          + New
                        </Btn>
                      </div>
                    ...
                    <td>
                      <input type="number" min={1} className="input text-right" value={l.qty}
                        onChange={e=>{
                          const n=parseInt(e.target.value||'1',10);
                          updateLine(l.id,{ qty: isNaN(n)||n<1 ? 1 : n });
                        }}/>
                    </td>
                    <td>
                      <input type="number" step="0.01" min={0} className="input text-right" value={(l.unitPrice/100).toFixed(2)}
                        onChange={e=>{
                          const f=parseFloat(e.target.value);
                          const cents=isNaN(f)?0:Math.round(f*100);
                          updateLine(l.id,{ unitPrice: Math.max(0,cents) });
                        }}/>
                    </td>
                    ...
                    <td className="text-right">
                      {(amt/100).toLocaleString(undefined,{style:'currency',currency:'USD'})}
                    </td>
                    <td className="text-center">
                      <Btn kind="ghost" className="text-xs px-2" onClick={()=>removeLine(l.id)}>✕</Btn>
                    </td>
                  </tr>
                );
              })}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <label className="block text-sm"><span>Message (shows on invoice)</span>
